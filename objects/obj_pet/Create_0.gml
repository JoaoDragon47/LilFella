...
carry_pet = false
is_aggro = true
pet_times_launched_to_aggro = 5
pet_times_launched = 0
...
cooldown_to_forgot_launch = FRAME * 10
...
check_grab_pet = function() {
	...
	if (_holding_mouse and (obj_mouse.pet_holding == noone or keyboard_check(vk_shift))) {
		obj_mouse.pet_holding = self
		carry_pet = true
		dir = 270
...
check_forgive_launch = function() {
	if (timer_to_forgot_launch > 0) {
		timer_to_forgot_launch--
		if (timer_to_forgot_launch <= 0) {
			pet_times_launched--
			
			if (pet_times_launched > 0) {
				timer_to_forgot_launch = cooldown_to_forgot_launch
			}
		}
	}
}
...
check_aggro_mode = function() {
	if (pet_times_launched >= pet_times_launched_to_aggro) {
...
state_taunt = new State()
state_taunt.start = function() {
	state_index = StatesIndex.Taunt
	sprite_base = Skins[skin_id][state_index]
}
state_taunt.step = function() {
	check_forgive_launch()
...
state_sit = new State()
state_sit.start = function() {
	state_index = StatesIndex.Sit
	sprite_base = Skins[skin_id][state_index]
	
	timer_to_change_state = FRAME * random_range(10, 20)
}
state_sit.step = function() {
	check_forgive_launch()
...
state_choose_state.start = function() {
	//next_state = choose(state_idle, state_walk, state_pet_go_steal_mouse)
	timer_to_change_state = FRAME * random_range(1, 10)
	
	var _weight = 0
	
	var i = 0; repeat(states_to_change_number) {
		_weight += states_to_change_naturally[i].weight
		
		states_options_array[i] = {
			state: struct_get(states_to_change_naturally[i], "state"),
			weight: _weight
		}
	...
	var _random_weight = irandom_range(1, _weight)
	
	i = 0; repeat(states_to_change_number) {
		if (_random_weight <= states_options_array[i].weight) {
			next_state = states_options_array[i].state
			break;
		}
...
state_pet_launched.start = function() {
	...
	state_index = StatesIndex.Launched
	sprite_base = Skins[skin_id][state_index]
}
...
state_pet_launched.stop = function() {
	...
	if (pet_times_launched == pet_times_launched_to_aggro) {
		is_aggro = true
		//file_Open("www.youtube.com/watch?v=Aq5WXmQQooo&pp=ygUJcmljayByb2xs", "") // Rick Roll
	}
}


state_pet_go_steal_mouse = new State()
state_pet_go_steal_mouse.start = function() {
	state_index = StatesIndex.Walk
	sprite_base = Skins[skin_id][state_index]
...
state_pet_go_steal_mouse.step = function() {
	sprite_base = Skins[skin_id][state_index]
...
state_pet_grab_mouse.start = function() {
	state_index = StatesIndex.StealMouse
	sprite_base = Skins[skin_id][state_index]
	
	x_dest = x + (choose(-1, 1) * (16 * choose(irandom_range(4, 10), irandom_range(-10, -4))))
	y_dest = y + (choose(-1, 1) * (16 * choose(irandom_range(4, 10), irandom_range(-10, -4))))
...
state_pet_grab_mouse.stop = function() {
	if (pet_times_launched > 0) {
		pet_times_launched = 0
		timer_to_forgot_launch = 0
	}
...
states_to_change_naturally = [
	{
		state: state_idle,
		weight: WEIGTH_STATE_COMMON
	},
	{
		state: state_walk,
		weight: WEIGTH_STATE_COMMON
	...
	{
		state: state_pet_go_steal_mouse,
		weight: WEIGTH_STATE_EPIC
	},
	{
		state: state_taunt,
		weight: WEIGTH_STATE_UNCOMMON
	},
	{
		state: state_sit,
		weight: WEIGTH_STATE_RARE
	}
]
states_to_change_number = array_length(states_to_change_naturally)
states_options_array = array_create(0)
