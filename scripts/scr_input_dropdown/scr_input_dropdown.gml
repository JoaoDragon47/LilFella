...
function input_box_dd_init(_left,_top,_right,_rowHeight,_xOffset,_yOffset,_font,_defaultOption,_startActive,_nextBox) {
	...
	top = _top*mScale;
	right = _right*mScale;
	rowHeight = _rowHeight*mScale;
	xOffset = _xOffset*mScale;
	yOffset = _yOffset*mScale;
	font = _font;
...
function input_box_dd_selection() {
	if (active == true) {
		...
		if (mx >= left && my >= top && mx <= right && my <= top+rowHeight+(rowHeight*array_length(dropdownOptions))) {
			if (mouseX != mx || mouseY != my) {
				mouseX = mx;
				mouseY = my;
				for (var _i=0; _i<array_length(dropdownOptions); _i++) {
					if (my >= top+rowHeight+(rowHeight*_i) && my <= top+rowHeight+(rowHeight*_i)+rowHeight) {
						selection = _i;
						break;
					}
				}
...
function input_box_dd_Global_Left_Pressed() {
	if (active == false) {
		var mx=display_mouse_get_x(),my=display_mouse_get_y();
		if (mx >= left && my >= top && mx <= right && my <= top+(rowHeight)) {
			active = true;
		}
		return false;
	}
