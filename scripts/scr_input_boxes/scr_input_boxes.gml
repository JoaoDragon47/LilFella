...
function input_box_init(_left,_top,_width,_height,_textYSep,_startActive,_defaultArray,_textWidth,_vbarInterval,_moveInterval,_font,_maxLines,_hideArray,_inputType,_excludedArray,_nextBox) {
	...
	top = _top*mScale;
	width = _width*mScale;
	height = _height*mScale;
	normalTextYSep = _textYSep*mScale;
	active = _startActive;
	...
	hideTextYSep = _hideArray[3]*mScale;
...
function input_box_step(_canCutCopy,_canPaste) {
	...
	if (active) {
		...
		if (mouse_check_button_pressed(mb_left)) {
			if (mx < left || my < top || mx > left+width || my > top+currentHeight) {//click away from input box
				input_set_inactive();
				...
				for (var _count=1; _pos<=string_length(drawInput); _count++) {
					...
					if (string_count("\n",string_copy(drawInput,_ind,_count)) > 0) {
						if (top+height+(_lines*textYSep) > my) {//mouse is at end of current line
							pos = _pos-1;
							break;
						}
						else {//reset string
					...
					if (left+string_width(_string) >= mx && top+height+(_lines*textYSep) > my) {//mouse at this position
						pos = _pos-1;
						break;
					}
					_pos += 1;
		...
		if (mouse_check_button(mb_left) && selecting == true && (mx >= left && my >= top && mx <= left+width && my <= top+currentHeight)) {
			//get pos in line
			var _ind = 1;
			var _pos = 1;
			var _lines = 0;
		...
		if (keyboard_lastkey == vk_backspace) {
			...
			if (endPos != -1) {//selected
				var _start = min(pos,endPos);
				var _end = max(pos,endPos);
				_input = string_delete(input,_start+1,_end-_start);
		...
		if (string_count("\n",input) == 0) {
			...
			if (_resetBreaks) {
				...
				if (_totalLines > 0) {
					var _pos = 0;
					var _stop = 0;
					while (_stop < _totalLines && string_pos_ext("\n",input,_pos) != 0 && string_pos_ext("\n",input,_pos) < pos+1) {
						_pos = string_pos_ext("\n",input,_pos);
						_startLine += 1;
						_stop += 1;
					}
		...
		if (mouse_check_button_pressed(mb_left)) {
			if (mx >= left && my >= top && mx <= left+width && my <= top+currentHeight) {
				input_set_active();
			}
		}
	}
...
function input_box_path_step(){
	var mx=display_mouse_get_x(),my=display_mouse_get_y();
	if (mouse_check_button_pressed(mb_left)) {
		if (mx >= left && my >= top && mx <= left+width && my <= top+currentHeight) {
			var _path=get_open_filename("image file|png","*.png");
			if(_path!=""){
				input=_path;
				drawInput=filename_name(_path);
			}
		}
	}
}
